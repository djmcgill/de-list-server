...
use hyper::header::{self, AUTHORIZATION, HeaderValue};
...
pub fn request_token<'a, 'b, S: Into<String>>(con_token: &'a KeyPair, callback: S, client: &'b Client<HttpsConnector<HttpConnector>, Body>)
                                      -> impl Future<Item=(), Error=Box<(dyn std::error::Error + Send + Sync +'b)>> {
//
...
//                            con_token, None, Some(callback.into()), None, None);
    let request = Request::connect::<Uri>(REQUEST_TOKEN.parse().unwrap())
        .header(AUTHORIZATION, HeaderValue::from_str("foo").unwrap())
        .method(Method::POST)
...
impl TwitterOAuth {
    fn fmt_scheme(&self) -> String { panic!() }
}
